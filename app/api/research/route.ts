    ...
    const { error } = await resend.emails.send({
      ...
      html: `
        <div style="font-family: Inter, sans-serif; max-width: 620px; margin: 0 auto; background: #ffffff; color: #0F172A; padding: 40px; border-radius: 12px; border: 1px solid #E2E8F0;">
          ...
          ${answersHtml || '<p style="color: #94A3B8; font-size: 13px;">No answers provided.</p>'}

          <p style="color: #CBD5E1; font-size: 11px; margin-top: 32px; text-align: center;">Submission ID: ${entry.id} · HireNove</p>
        </div>
      `,
    });
